...
const EditStoreScreen = () => {
  ...
  const [existingStoreData, setExistingStoreData] = useState(null);
    ...
    const fetchStoreData = async () => {
      try {
        ...
        if (user) {
          ...
          if (docSnap.exists()) {
            setExistingStoreData(docSnap.data());
            setStoreName(docSnap.data().storeName);
            setStoreLocation(docSnap.data().storeLocation);
            setAboutUs(docSnap.data().aboutUs);
            setProfileImageUri(docSnap.data().profileImageURL);
  ...
  const handleEditStore = async () => {
    ...
    try {
      ...
      if (profileImage !== existingStoreData?.profileImageURL) {
      ...
      const storeRef = doc(db, 'stores', user.uid); // Reference the document with the user's ID
  
      // Update the store details in Firestore
      await updateDoc(storeRef, {
        ...
        latitude,
        longitude,
        updatedAt: new Date(), // Keep track of the last update time
      });
